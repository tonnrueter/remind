...
cat(
  date(), "climateAssessmentImpulseResponse.R:", reportClimateAssessmentConfig(cfg),
...
# Note: Emissions impulse is added to the harmonized & infilled data from a previous climate assessment interim run
...
if (!file.exists(cfg$harmInfEmissionsFile)) {
  errMsg <- paste(
    date(), "climateAssessmentImpulseResponse.R:", cfg$harmInfEmissionsFile,
    "(emissions after harmonization and infilling) file not found\n"
  )
  cat(errMsg, file = cfg$logFile, append = TRUE)
  stop(errMsg)
}
mifBaseScen <- read.quitte(cfg$harmInfEmissionsFile)
...
# `write.IAMCcsv` appends a artificial column for some reason and does weird rounding
mifAllPulsesScen %>%
...
file.exists(cfg$climateAssessmentFile)
if (!file.exists(cfg$climateAssessmentFile)) {
  stop(date(), "climateAssessmentImpulseResponse.R: Climate assessment output file not found")
}
cat(
  date(), "climateAssessmentImpulseResponse.R: Produced", cfg$climateAssessmentFile, "\n",
  file = cfg$logFile, append = TRUE
)
...
if (cfg$isArchived) {
  ...
  extras <- c(normalizePath(file.path(outputDir, paste0("pm_magicc_temperatureImpulseResponse.gdx")), mustWork = TRUE))
  ...
  cat(
    date(), "climateAssessmentImpulseResponse.R: Archived climate assessment data to", tarfile, "\n",
    file = cfg$logFile, append = TRUE
  )
